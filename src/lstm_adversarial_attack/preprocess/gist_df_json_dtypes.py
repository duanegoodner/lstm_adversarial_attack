...
df_icustay = rio.ResourceImporter().import_pickle_to_df(
    path=cfp.PREFILTER_OUTPUT / "icustay.pickle"
)


a_start = time.time()
df_to_typed_json_a(df=df_icustay, path=Path("df_a.json"))
a_mid = time.time()
re_imported_df_a = df_from_typed_json_a(typed_json=Path("df_a.json"))
a_end = time.time()

b_start = time.time()
df_to_typed_json_b(df=df_icustay, path=Path("df_b.json"))
b_mid = time.time()
